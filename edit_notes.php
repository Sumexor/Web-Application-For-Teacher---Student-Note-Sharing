...
<?php
$note = $_GET['note'];
$qrr = "SELECT * FROM mynotes WHERE note_name='$note' && id='$id'";
$data = mysqli_query($con,$qrr);
$row = mysqli_fetch_assoc($data);
echo "<input type='text' placeholder='Note Title' name='title' id='title' class='form-control mb-5' value='" . $row['note_name'] ."' required>";
echo "<textarea id='my-summernote' name='editordata' id='data' required>". $row['data'] . "</textarea>";
echo "<input type='submit' value='Submit' name='submit' class='btn btn-success mt-3'></input>";
echo "<input type='submit' value='Save' name='save' class='btn btn-info mt-3'></input>";
?>
</form>
...
<?php
	...
	$data=mysqli_query($con,$qrr);
      ...
      if($rows>0){
		$stat = "published";
        $qrr = "UPDATE mynotes SET data='$ndata',status='$stat' WHERE note_name='$title' && id='$id'";
		$data = mysqli_query($con,$qrr);
        ...
        echo "<script>alert('Note not available you cannot update!.(tip:- Do not change note name!)');</script>";
      }
    }
	...
	if($ndata){
        ...
        $stat = "non-public";
        $qrr1 = "UPDATE mynotes SET data='$ndata',status='$stat' WHERE note_name='$title' && id='$id'";
